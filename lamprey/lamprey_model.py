...
def phi(z,m):
    return np.exp(-np.square(z-(m/10.))/np.square(0.5))

def Phi(z):
    return [1., np.sin(2.*np.pi*z), np.cos(2.*np.pi*z), np.sin(4.*np.pi*z)]
...
with nengo.Network('basic_lamprey', seed=1) as model:
    ...
    a = nengo.Ensemble(n_neurons=500, dimensions=3,
        radius=1,noise=white_noise, label="cpg")
    ...
    def stim_func(t):
        return np.array([1., 1., 1.]) if t < 1.0 else np.array([0, 0, 0])
    ### end stim_func        
